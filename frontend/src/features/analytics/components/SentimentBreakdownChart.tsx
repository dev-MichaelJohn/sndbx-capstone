...
/**
 * Donut chart component representing student qualitative feedback sentiment breakdown.
 * Defaults gracefully to Neutral when no comments are present.
 */
export const SentimentBreakdownChart = ({ sentimentBreakdown }: SentimentBreakdownChartProps) => {
  const sentimentData = sentimentMeta.map(({ key, label, color }) => ({
    name: label,
    value: sentimentBreakdown?.[key] ?? 0,
    fill: color,
  }));

  const hasData = sentimentData.some((d) => d.value > 0);
  const dominant = hasData
    ? sentimentData.reduce((a, b) => (a.value > b.value ? a : b))
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <CardDescription className="mt-0.5">
              Qualitative evaluation sentiment proportions (AFINN NLP)
            </CardDescription>
          </div>
        </div>
      </CardHeader>
        ...
        {!hasData ? (
          <div className="flex h-64 w-full flex-col items-center justify-center gap-3 rounded-lg border border-dashed border-border/60 bg-muted/20">
            ...
            <div className="text-center">
              <p className="text-sm font-medium text-muted-foreground">Neutral / No Feedback</p>
              <p className="text-xs text-muted-foreground/60 mt-0.5">
                Sentiment will populate as students provide qualitative feedback.
              </p>
            </div>
          </div>
        ) : (
