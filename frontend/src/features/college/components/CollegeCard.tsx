...
import { Building2, User, ArrowRight, MoreHorizontal, Pencil, Trash2 } from "lucide-react";
  ...
  return (
    <Card className="group relative flex flex-col justify-between overflow-hidden rounded-xl border border-border/60 bg-card shadow-2xs transition-all duration-200 hover:-translate-y-1 hover:border-primary/40 hover:shadow-md">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-40 rounded-xl p-1">
              ...
              <DropdownMenuItem
                ...
                onSelect={(e) => e.preventDefault()}
              >
                <CollegeDeleteDialog college={college} icon={Trash2} triggerText="Delete" />
              </DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
        </div>
          ...
          {deanName ? (
            <div className="flex items-center gap-2.5">
              ...
              <div className="min-w-0 flex-1">
                <p className="text-xs font-semibold text-foreground truncate">{deanName}</p>
      ...
      <CardFooter className="border-t border-border/40 p-3 px-5 bg-muted/10 flex items-center justify-between">
        <span className="text-[11px] text-muted-foreground">Degree Programs</span>
        <Button
          asChild
          size="sm"
          variant="ghost"
          className="h-7 text-xs font-semibold gap-1.5 text-primary hover:text-primary hover:bg-primary/10 rounded-lg cursor-pointer"
        >
