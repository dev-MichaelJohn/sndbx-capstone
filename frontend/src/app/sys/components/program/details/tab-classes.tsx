...
import { useClasses, useDeleteClass } from "@/features/sys/class.service";
import { ClassCreateDialog } from "../../class/class-create"; // Adjust path as needed
import { DataTable } from "@/components/data-table";
import { getClassColumns } from "../../class/class-columns"; // Adjust path as needed
...
export const ClassesTab = ({ programId }: ClassesTabProps) => {
  ...
  const handleDelete = (id: number, identifier: string) => {
    if (confirm(`Are you sure you want to remove Class ${identifier}?`)) {
      deleteClass(id);
    }
  };

  const columns = getClassColumns({ onDelete: handleDelete, isDeleting });
