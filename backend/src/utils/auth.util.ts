...
import { JWTPayloadSchema, type JWTPayloadType } from "@/services/token.service.js";
 ...
 */
export const OTPAuthStrategy = new OTPStrategy(async (
  req: Request, done: (error?: any, user?: Express.User | false, options?: IVerifyOptions) => void
) => {
  try {
...
}, async (payload, done: (error?: any, user?: Express.User | false, options?: IVerifyOptions) => void) => {
  try {
