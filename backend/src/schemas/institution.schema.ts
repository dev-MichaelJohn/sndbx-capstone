...
import { check, date, index, integer, pgEnum, pgTable, serial, smallint, timestamp, uniqueIndex, varchar } from "drizzle-orm/pg-core";
...
export const Colleges = pgTable("colleges", {
  ...
  uniqueIndex("uidx_active_college_name").on(t.name)
    .where(sql`deleted_at IS NULL`),
  uniqueIndex("uidx_active_college_initialism").on(t.initialism)
    .where(sql`deleted_at IS NULL`),
]);

export const Programs = pgTable("programs", {
  id: serial("id").primaryKey(),
  college_id: integer("college_id").notNull().references(() => Colleges.id),
  name: varchar("name", { length: 128 }).notNull(),
  initialism: varchar("initialism", { length: 16 }).notNull(),
  created_at: timestamp("created_at").notNull().defaultNow(),
  updated_at: timestamp("updated_at").notNull().defaultNow().$onUpdate(() => new Date()),
  ...
  uniqueIndex("uidx_active_program_college_name").on(t.college_id, t.name)
    .where(sql`deleted_at IS NULL`),
  uniqueIndex("uidx_active_program_name_initialism").on(t.initialism)
    .where(sql`deleted_at IS NULL`),
]);
...
export const Semesters = pgTable("semesters", {
  ...
  school_year_end: smallint("school_year_end").notNull().generatedAlwaysAs((): SQL => sql`${Semesters.school_year_start} + 1`),
...
export const CollegeDeans = pgTable("college_deans", {
  id: serial("id").primaryKey(),
  college_id: integer("college_id").notNull().references(() => Colleges.id),
  dean_id: integer("dean_id").notNull().references(() => Accounts.id),
  created_at: timestamp("created_at").notNull().defaultNow(),
  updated_at: timestamp("updated_at").notNull().defaultNow().$onUpdate(() => new Date()),
  deleted_at: timestamp("deleted_at"),
}, (t) => [
  uniqueIndex("uidx_active_college_dean").on(t.college_id)
    .where(sql`deleted_at IS NULL`),
  uniqueIndex("uidx_active_dean_account").on(t.dean_id)
    .where(sql`deleted_at IS NULL`),
...
export const ProgramChairs = pgTable("program_chairs", {
  id: serial("id").primaryKey(),
  program_id: integer("program_id").notNull().references(() => Programs.id),
  chair_id: integer("chair_id").notNull().references(() => Accounts.id),
  created_at: timestamp("created_at").notNull().defaultNow(),
  updated_at: timestamp("updated_at").notNull().defaultNow().$onUpdate(() => new Date()),
  deleted_at: timestamp("deleted_at"),
}, (t) => [
  uniqueIndex("uidx_active_program_chair").on(t.program_id)
    .where(sql`deleted_at IS NULL`),
...
export const ClassCourses = pgTable("class_courses", {
  ...
  faculty_id: integer("faculty_id").notNull().references(() => Accounts.id),
  created_at: timestamp("created_at").notNull().defaultNow(),
  updated_at: timestamp("updated_at").notNull().defaultNow().$onUpdate(() => new Date()),
  deleted_at: timestamp("deleted_at"),
}, (t) => [
...
export const StudentClasses = pgTable("student_classes", {
  id: serial("id").primaryKey(),
  student_account_id: integer("student_account_id").notNull().references(() => Accounts.id),
  class_course_id: integer("class_course_id").notNull().references(() => ClassCourses.id),
  created_at: timestamp("created_at").notNull().defaultNow(),
  updated_at: timestamp("updated_at").notNull().defaultNow().$onUpdate(() => new Date()),
  deleted_at: timestamp("deleted_at"),
}, (t) => [
